...
def test(model, device, loader, ppi_adj, new_ppi_features, pro_graph):
    model.eval()
    ...
    with torch.no_grad():
        for data in loader:
            ...
            output = model(mol_data, pro_data, ppi_adj, new_ppi_features, pro_graph)
...
def main(args):
    ...
    G, P = test(model, device, test_loader, ppi_adj, new_ppi_features, pro_graph)
